	...
	<nav class="navbar navbar-expand-md navbar-dark bg-dark fixed-top">
      ...
      <div class="collapse navbar-collapse" id="navbarsExampleDefault">
        <ul class="navbar-nav mr-auto">
          ...
          <li class="nav-item">
            <a class="nav-link" href="{{route('NewCategory')}}">Nueva Categoría</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="{{route('NewProduct')}}">Nuevo Producto</a>
          </li>
        </ul>
      </div>
    </nav>
    ...
    <main role="main" class="container">
  		<div class="album py-5 bg-light">
  			<div class="container">
				...
				<button type="button" class="btn btn-primary" onclick="location.href='{{route('NewCategory')}}';">Añade una nueva Categoría</button>
				...
				<table class="table table-striped">
					...
					@foreach($products as $product)
						...
						<tr class="text-center align-middle">
							...
							<td class="align-middle"><button type="button" class="btn btn-sm btn-success" disabled>{{ $product->price.'€' }}</button></td>
							
							@foreach($categories as $category)
                          
                          		@if($product->category_id == $category->id)

									<td class="align-middle"><button type="button" class="btn btn-sm btn-primary" disabled>{{ $category->name }}</button></td>

								@endif

							@endforeach

							<td class="align-middle">{{ HTML::image('img/products/'.$product->image, $product->name, array('width' => '50', 'height' => '50')) }}</td>
							<td class="align-middle">
				...
				<button type="button" class="btn btn-primary" onclick="location.href='{{route('NewProduct')}}';">Añade un nuevo Producto</button>
